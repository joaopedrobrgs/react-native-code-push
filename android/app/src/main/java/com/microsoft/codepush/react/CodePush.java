...
public class CodePush implements ReactPackage {
  ...
  public CodePush(String deploymentKey, Context context, boolean isDebugMode) {
    ...
    mUpdateManager =
      new CodePushUpdateManager(context.getFilesDir().getAbsolutePath());
    ...
    if (sAppVersion == null) {
      try {
        PackageInfo pInfo = mContext
          .getPackageManager()
          .getPackageInfo(mContext.getPackageName(), 0);
        sAppVersion = pInfo.versionName;
      } catch (PackageManager.NameNotFoundException e) {
        throw new CodePushUnknownException(
          "Unable to get package info for " + mContext.getPackageName(),
          e
        );
      }
    }
    ...
    String serverUrlFromStrings = getCustomPropertyFromStringsIfExist(
      "ServerUrl"
    );
  ...
  public CodePush(
    ...
    if (publicKeyResourceDescriptor != null) {
      mPublicKey =
        getPublicKeyByResourceDescriptor(publicKeyResourceDescriptor);
    }

    mServerUrl = serverUrl;
  }

  private String getPublicKeyByResourceDescriptor(
    int publicKeyResourceDescriptor
  ) {
    String publicKey;
    try {
  ...
  private String getCustomPropertyFromStringsIfExist(String propertyName) {
    ...
    int resId = mContext
      .getResources()
      .getIdentifier("CodePush" + propertyName, "string", packageName);
  ...
  private boolean isLiveReloadEnabled(ReactInstanceManager instanceManager) {
    ...
    if (instanceManager != null) {
      ...
      if (devSupportManager != null) {
        Method[] methods = devSupportManager
          .getDevSettings()
          .getClass()
          .getMethods();
  ...
  public void clearDebugCacheIfNeeded(ReactInstanceManager instanceManager) {
    if (
      ...
      !isLiveReloadEnabled(instanceManager)
    ) {
      // This needs to be kept in sync with https://github.com/facebook/react-native/blob/master/ReactAndroid/src/main/java/com/facebook/react/devsupport/DevSupportManager.java#L78
      File cachedDevBundle = new File(
  ...
  long getBinaryResourcesModifiedTime() {
    try {
      ...
      int codePushApkBuildTimeId =
        this.mContext.getResources()
          .getIdentifier(
            CodePushConstants.CODE_PUSH_APK_BUILD_TIME_KEY,
            "string",
            packageName
          );
      // replace double quotes needed for correct restoration of long value from strings.xml
      // https://github.com/microsoft/cordova-plugin-code-push/issues/264
      String codePushApkBuildTime =
        this.mContext.getResources()
          .getString(codePushApkBuildTimeId)
          .replaceAll("\"", "");
  ...
  public String getPackageFolder() {
    ...
    return mUpdateManager.getPackageFolderPath(
      codePushLocalPackage.optString("packageHash")
    );
  }
  ...
  public String getJSBundleFileInternal(String assetsBundleFileName) {
    ...
    try {
      packageFilePath =
        mUpdateManager.getCurrentPackageBundlePath(this.mAssetsBundleFileName);
  ...
  void initializeUpdateAfterRestart() {
    ...
    if (pendingUpdate != null) {
      ...
      if (
        ...
        !isPackageBundleLatest(packageMetadata) &&
        hasBinaryVersionChanged(packageMetadata)
      ) {
        CodePushUtils.log(
      ...
      try {
        boolean updateIsLoading = pendingUpdate.getBoolean(
          CodePushConstants.PENDING_UPDATE_IS_LOADING_KEY
        );
        if (updateIsLoading) {
        ...
        // Should not happen.
        throw new CodePushUnknownException(
          "Unable to read pending update metadata stored in SharedPreferences",
          e
        );
      }
    }
  ...
  private boolean isPackageBundleLatest(JSONObject packageMetadata) {
    try {
      ...
      String binaryModifiedDateDuringPackageInstallString = packageMetadata.optString(
        CodePushConstants.BINARY_MODIFIED_TIME_KEY,
        null
      );
      if (binaryModifiedDateDuringPackageInstallString != null) {
        binaryModifiedDateDuringPackageInstall =
          Long.parseLong(binaryModifiedDateDuringPackageInstallString);
      }
      ...
      return (
        binaryModifiedDateDuringPackageInstall != null &&
  ...
  public static void setUsingTestConfiguration(
    boolean shouldUseTestConfiguration
  ) {
  ...
  public static void setReactInstanceHolder(
    ReactInstanceHolder reactInstanceHolder
  ) {
  ...
  public List<NativeModule> createNativeModules(
    ...
    CodePushNativeModule codePushModule = new CodePushNativeModule(
